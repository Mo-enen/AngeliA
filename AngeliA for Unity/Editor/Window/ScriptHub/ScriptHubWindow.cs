...
namespace AngeliaForUnity.Editor {
	...
	public class ScriptHubWindow : EditorWindow {
		...
		private class Item {
			...
			public bool IsFile;
			public Item (string name, string path, bool isFile) {
				Name = name;
				Path = path;
				IsFile = isFile;
			}
		...
		private class WindowStyle {
			...
			public Texture2D DefaultFileIcon = EditorGUIUtility.IconContent("DefaultAsset Icon").image as Texture2D;
			public Texture2D ArtworkFileIcon = EditorGUIUtility.IconContent("RenderPipelineAsset Icon").image as Texture2D;
			public Texture2D ConversationFileIcon = EditorGUIUtility.IconContent("TextScriptImporter Icon").image as Texture2D;
		}
		...
		private static void Init () {
			EditorApplication.playModeStateChanged += (mode) => {
				if (mode == PlayModeStateChange.EnteredEditMode) {
					FocusWindowIfItsOpen<ScriptHubWindow>();
				}
			};
		}
		...
		private void OnGUI () {
			...
			MGUI.Rect(0, 1);
			MGUI.CancelFocusOnClick(this, true);
				...
				using (new GUILayout.HorizontalScope()) {
					for (int col = 0; col < column && i < scripts.Length; col++, i++) {
						...
						if (item.IsFile) {
							// File
							if (icon == null) {
								icon = AssetPreview.GetMiniThumbnail(AssetDatabase.LoadAssetAtPath<Object>(path));
								if (icon == null) {
									string ext = Util.GetExtension(item.Path);
									icon =
										ext == ".ase" || ext == ".aseprite" ? Style.ArtworkFileIcon :
										ext == $".{AngePath.EDITABLE_CONVERSATION_FILE_EXT}" ? Style.ConversationFileIcon :
										Style.DefaultFileIcon;
								}
								item.Icon = icon;
							}
			...
			void LoadScripts (
				...
				foreach (var pt in searchPats) {
					if (!string.IsNullOrEmpty(pt.Label)) {
						scripts.Add(new Item(pt.Label, "", false));
					}
				...
				void LoadAll (string folderFullPath, int depth, bool addFolderTags, bool ignore, params string[] search) {
					...
					if (!ignore && addFolderTags) {
						if (scripts.Count == 0 || scripts[^1].IsFile) {
							scripts.Add(new Item(getFolderName(folderName), folderFullPath, false));
						} else {
							scripts[^1] = new Item(getFolderName(folderName), folderFullPath, false);
						}
					}
					...
					foreach (var file in files) {
						...
						var item = new Item(
							Util.GetDisplayName(getFileName(Util.GetNameWithoutExtension(path))), path, true
						);
