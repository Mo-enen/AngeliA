...
namespace Yaya {
	public class eConveyorLeft : eConveyor {
		...
		public override void FrameUpdate (int frame) {
			...
			switch (Part) {
				case PartType.LeftEdge:
					CellRenderer.Draw(LEFT_CODES[(frame * Mathf.Abs(MovingSpeed) / 16) % 8], Rect);
					break;
				case PartType.Middle:
					CellRenderer.Draw(MID_CODES[(frame * Mathf.Abs(MovingSpeed) / 16) % 8], Rect);
					break;
				case PartType.RightEdge:
					CellRenderer.Draw(RIGHT_CODES[(frame * Mathf.Abs(MovingSpeed) / 16) % 8], Rect);
					break;
				case PartType.Single:
					CellRenderer.Draw(SINGLE_CODES[(frame * Mathf.Abs(MovingSpeed) / 16) % 8], Rect);
					break;
			}
