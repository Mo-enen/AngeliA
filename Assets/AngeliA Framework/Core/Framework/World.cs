...
namespace AngeliaFramework {
	...
	public abstract class WorldGenerator {
		public abstract void FillWorld (World world);
	}
	...
	public class World {
		...
		public struct Block {
			...
			public bool HasCollider => ColliderBorder.Left >= 0;
					...
					while (reader.NotEnd()) {
						...
						if (x >= 0) {
							...
							if (y >= 0) {
								...
								int borderD = -1;
								int borderU = -1;
								bool hasCollider = borderL >= 0;
								if (hasCollider) {
									borderR = reader.ReadInt32();
									borderD = reader.ReadInt32();
									borderU = reader.ReadInt32();
								}
								if (x < 0 || x >= SIZE || y < 0 || y >= SIZE) continue;
		...
		public int EditorOnly_SaveToDisk (int worldX, int worldY, int instanceID) {
			...
			for (int y = 0; y < SIZE; y++) {
				for (int x = 0; x < SIZE; x++) {
					...
					if (block.ColliderBorder.Left >= 0) {
