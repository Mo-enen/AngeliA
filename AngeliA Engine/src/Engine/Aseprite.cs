...
internal class Aseprite {
	...
	public Color32[] GetPalette32 () {
		...
		if (Header.ColorDepth == 8 && Header.PaletteEntry < paletteLen) {
			colors[Header.PaletteEntry] = Color32.CLEAR;
		}
		return colors;
	}
			...
			switch (colorDepth) {
				...
				case 16:
					...
					for (int i = 0; i < colors.Length; i++) {
						...
						rgb = rawBytes[_i * 2 + 0];
						a = rawBytes[_i * 2 + 1];
						colors[i] = new Color32(rgb, rgb, rgb, a);
					}
					break;
				case 8:
					byte index;
					for (int i = 0; i < colors.Length; i++) {
						int _i = (Height - i / Width - 1) * Width + (i % Width);
						index = rawBytes[_i];
						colors[i] = index < palette.Length ? palette[index] : Color32.CLEAR;
					}
					break;
			}
			return colors;
