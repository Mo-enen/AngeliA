...
public static partial class FrameworkUtil {
	...
	// Draw Tool/Weapon
	public static void DrawWeaponDistribute (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite, int handheldAnimationID) {
		switch (handheldAnimationID) {
			case var _ when handheldAnimationID == PoseHandheld_Bow.TYPE_ID:
				DrawWeapon_Bow(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_Double.TYPE_ID:
				DrawWeapon_DoubleHanded(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_EachHand.TYPE_ID:
				DrawWeapon_EachHanded(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_Float.TYPE_ID:
				DrawWeapon_Float(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_MagicPole.TYPE_ID:
				DrawWeapon_Polearm(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_Pole.TYPE_ID:
				DrawWeapon_Polearm(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_Shooting.TYPE_ID:
				DrawWeapon_Shoot(tool, renderer, sprite);
				break;
			case var _ when handheldAnimationID == PoseHandheld_SingleHanded.TYPE_ID:
				DrawWeapon_SingleHanded(tool, renderer, sprite);
				break;
		}
	}

	public static void DrawTool_Block (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
	...
	public static void DrawWeapon_Float (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
	...
	public static void DrawWeapon_SingleHanded (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		bool attacking = renderer.TargetCharacter.Attackness.IsAttacking;
	...
	public static void DrawWeapon_DoubleHanded (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		bool attacking = renderer.TargetCharacter.Attackness.IsAttacking;
		int twistR = attacking && !tool.IgnoreGrabTwist ? renderer.HandGrabAttackTwistR : 1000;
		int facingSign = renderer.TargetCharacter.Movement.FacingRight ? 1 : -1;
		int grabScale = renderer.HandGrabScaleR;
	...
	public static void DrawWeapon_Throwing (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		bool attacking = renderer.TargetCharacter.Attackness.IsAttacking;
		int twistR = attacking && !tool.IgnoreGrabTwist ? renderer.HandGrabAttackTwistR : 1000;
		int facingSign = renderer.TargetCharacter.Movement.FacingRight ? 1 : -1;
		int grabRotation = renderer.HandGrabRotationR;
	...
	public static void DrawWeapon_Shoot (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
	...
	public static void DrawWeapon_EachHanded (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		bool attacking = renderer.TargetCharacter.Attackness.IsAttacking;
	...
	public static void DrawWeapon_Polearm (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		var centerL = renderer.HandL.GlobalLerp(0.5f, 0.5f);
		var centerR = renderer.HandR.GlobalLerp(0.5f, 0.5f);
		int twistR = renderer.TargetCharacter.Attackness.IsAttacking && !tool.IgnoreGrabTwist ? renderer.HandGrabAttackTwistR : 1000;
		tool.OnToolSpriteRendered(
	...
	public static void DrawWeapon_Bow (HandTool tool, PoseCharacterRenderer renderer, AngeSprite sprite) {
		...
		if (attack.IsAttacking) {
			...
			if (!sprite.IsTrigger) {
				...
				if (localFrame < tool.Duration / 2) {
					// Pulling
					float ease01 = Ease.OutQuad(localFrame / (tool.Duration / 2f));
					width += Util.LerpUnclamped(0, width * 2 / 3, ease01).RoundToInt();
					height -= Util.LerpUnclamped(0, height / 2, ease01).RoundToInt();
				} else {
					// Release
					float ease01 = Ease.OutQuad((localFrame - tool.Duration / 2f) / (tool.Duration / 2f));
