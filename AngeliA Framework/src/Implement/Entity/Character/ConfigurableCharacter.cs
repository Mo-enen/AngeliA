...
public interface IConfigurableCharacter {
	...
	public sealed void LoadCharacterFromConfig () {
		...
		if (Config == null) {
			var uni = UniverseSystem.CurrentUniverse;
			Config = JsonUtil.LoadOrCreateJsonFromPath<CharacterConfig>(
				Util.CombinePaths(uni.SavingMetaRoot, "Character Config", $"{GetType().Name}.json")
			);
		}
