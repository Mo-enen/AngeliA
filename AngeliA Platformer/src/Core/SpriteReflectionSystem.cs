...
public static class SpriteReflectionSystem {
	...
	// VAR
	private static readonly Dictionary<int, int> SpritePool = [];
	...
	internal static void OnGameUpdateLater () {
		...
		using var _ = new LayerScope(RenderLayer.REFLECTION);
		...
		for (int i = 0; i < count; i++) {
			var cell = cells[i];
			if (cell.Sprite == null || !SpritePool.TryGetValue(cell.Sprite.ID, out int amount)) continue;
			var rect = new IRect(
			...
			DrawReflection(rect, target, size, cell.Sprite.SummaryTint);
			hasReflection = true;
		}
		if (hasReflection) {
			Renderer.SetLayerTint(RenderLayer.REFLECTION, Color32.WHITE.WithNewA(128));
		}
	}

	// API
	public static void RegisterSprite (int spriteID, int amount) {
		// Sprite
		if (Renderer.TryGetSprite(spriteID, out var sprite)) {
			SpritePool[spriteID] = amount;
		}
		// Group
		if (Renderer.TryGetSpriteGroup(spriteID, out var group)) {
			foreach (var sp in group.Sprites) {
				SpritePool[sp.ID] = amount;
			}
		}
#if DEBUG
		// Warning
	...
	// LGC
	private static void DrawReflection (IRect source, IRect target, int size, Color32 tint) {
		...
		var rect = new IRect(0, 0, size, size);
		for (int j = d; j <= u; j++) {
			for (int i = l; i <= r; i++) {
				...
				if (blocks.bg != 0) {
					if (Renderer.TryGetSpriteForGizmos(blocks.bg, out var sp)) {
						var cell = Renderer.Draw(sp, rect, tint);
						cell.Clamp(source);
					}
